...
const OBJECTIFS = [
  ...
  },
  {
    ...
    accompli: function(e) { return e.basicWoodPlanksTotalProduit >= 1; }
  },
...
function reinitialiserProgressionRecette(slot, retirerRecette) {
  if (!slot) return;
  if (typeof campTutorialAutoriserMutationSlot !== "undefined"
      && !campTutorialAutoriserMutationSlot(slot)) return false;
...
const CAMP_APPEAL_RULES = Object.freeze({
  ...
  decimalPrecision: 6,
  fixedItems: Object.freeze({
    "camp-initial-tree-2": Object.freeze({ appeal: 2, connectable: false, displayLabel: "Old Tree" })
  }),
  itemTypes: Object.freeze({
    cardboardBox: Object.freeze({
      requireBuilt: true,
      connectable: true,
      tiers: Object.freeze({ 2: 1 })
    })
  })
});

function normaliserValeurAppeal(value) {
  const factor = Math.pow(10, CAMP_APPEAL_RULES.decimalPrecision);
  return Math.round((Number(value) || 0) * factor) / factor;
}

function appealCampDebloque() {
  const progression = progressionCamp();
  return progression.appealUnlocked === true;
}

function regleAppealItemCamp(item) {
  if (!item) return null;
  const fixed = CAMP_APPEAL_RULES.fixedItems[item.uid];
  if (fixed) return fixed;
  const typeRule = CAMP_APPEAL_RULES.itemTypes[item.type];
  if (!typeRule || (typeRule.requireBuilt && item.construit !== true)) return null;
  const appeal = Number(typeRule.tiers[String(Math.max(1, Number(item.tier) || 1))]);
  if (!Number.isFinite(appeal) || appeal <= 0) return null;
  return { appeal: appeal, connectable: typeRule.connectable === true };
}
...
function itemCampAdjacentBasicTrail(item, layout) {
  ...
  return (Array.isArray(layout) ? layout : []).some(function(candidate) {
    ...
    return occupied.has(candidate.x + ":" + (candidate.y - 1))
      || occupied.has((candidate.x + 1) + ":" + candidate.y)
      || occupied.has(candidate.x + ":" + (candidate.y + 1))
      || occupied.has((candidate.x - 1) + ":" + candidate.y);
  });
}
...
function scoreAttractiviteCamp() {
  ...
  const items = (Array.isArray(campPrototypeLayout) ? campPrototypeLayout : []).map(function(item) {
    ...
    return {
      uid: item.uid,
      type: item.type,
      tier: Math.max(1, Number(item.tier) || 1),
      label: fixed && fixed.displayLabel ? fixed.displayLabel : (type ? type.label : item.type),
      base: normaliserValeurAppeal(rule.appeal),
      connected: connected,
      value: value
    };
  }).filter(Boolean);
...
function detailsAttractiviteCamp() {
  return scoreAttractiviteCamp().items;
}

function formaterNombreAppealUi(value, maximumDecimals) {
  const decimals = Math.max(0, Number(maximumDecimals) || 0);
  return Number(value || 0).toFixed(decimals).replace(/\.?0+$/, "") || "0";
}

function formaterMultiplicateurAppealUi(value) {
  const parts = Number(value || 0).toFixed(3).split(".");
  while (parts[1].length > 2 && parts[1].endsWith("0")) parts[1] = parts[1].slice(0, -1);
  return parts.join(".");
}

function sourcesAppealCamp(score) {
  const groupes = new Map();
  (score && Array.isArray(score.items) ? score.items : []).forEach(function(item) {
    const label = item.type === "cardboardBox"
      ? "Cardboard Box T" + item.tier
      : item.label;
    const groupe = groupes.get(label) || { label: label, count: 0, value: 0 };
    groupe.count += 1;
    groupe.value = normaliserValeurAppeal(groupe.value + item.base);
...
function renduDetailsAttractiviteCamp() {
  ...
  const multiplicateur = multiplicateurVitesseAppealCamp(score.total);
  const gainPourcent = normaliserValeurAppeal((multiplicateur - 1) * 100);
  const sources = sourcesAppealCamp(score);
  panneau.innerHTML = '<strong>Appeal: ' + formaterNombreAppealUi(score.total, 6) + '</strong>'
    + '<span class="camp-appeal-speed"><b>Recruit Speed</b><em>×'
    + formaterMultiplicateurAppealUi(multiplicateur) + ' (+'
    + formaterNombreAppealUi(gainPourcent, 1) + '%)</em></span>'
    + '<span class="camp-appeal-rule">Each Appeal multiplies Recruit Speed by ×'
    + CAMP_APPEAL_RULES.recruitmentSpeedBase.toFixed(2) + '.</span>'
    + '<span class="camp-appeal-detail-section"><b>Appeal sources</b>'
    + (sources.length
      ? '<span>' + sources.map(function(source) {
          return '<i>' + echapperAttributHtml(source.label)
            + (source.count > 1 ? ' ×' + source.count : '')
            + '<em>+' + formaterNombreAppealUi(source.value, 6) + '</em></i>';
        }).join("") + '</span>'
...
function multiplicateurVitesseAppealCamp(appeal) {
  if (!appealCampDebloque()) return 1;
  return Math.pow(
    CAMP_APPEAL_RULES.recruitmentSpeedBase,
    Math.max(0, Number(appeal) || 0)
  );
}
...
function catHouseDebloquee()        { return progressionCamp().woodCathouseUnlocked === true; }
...
function ouvrirDialogueModal(id, options) {
  const opened = dialoguesController.open(id, options);
  if (typeof mobileInputDiagnostic !== "undefined") {
    mobileInputDiagnostic.recordOutcome("modal.open", { id: id, opened: opened !== false });
  }
  return opened;
}

let verificationStoryModalsProgrammee = false;

function programmerVerificationStoryModalsApresFermeture() {
  if (verificationStoryModalsProgrammee || !appealIntroEnAttente()) return;
  verificationStoryModalsProgrammee = true;
  Promise.resolve().then(function() {
    verificationStoryModalsProgrammee = false;
    verifierStoryModals();
  });
}

function fermerDialogueModal(id) {
  const closed = dialoguesController.close(id);
  if (typeof mobileInputDiagnostic !== "undefined") {
    mobileInputDiagnostic.recordOutcome("modal.close", { id: id, closed: closed !== false });
  }
  if (closed !== false) programmerVerificationStoryModalsApresFermeture();
  return closed;
}
...
function renduSequence() {
  ...
  document.body.classList.toggle("camp-appeal-unlocked", appealCampDebloque());
...
function forcerKittySlot(kittyIndex) {
  ...
  if (retirerKittyDeSesRoles(kittyIndex) === false) return false;
...
function forcerWorkerRecette(kittyIdx, familyId, slotIdx) {
  if (typeof campTutorialAutoriserIntent !== "undefined"
      && !campTutorialAutoriserIntent({ type: "worker", familyId: familyId, slotIndex: slotIdx })) return false;
  const guidanceDescriptor = typeof guidanceController !== "undefined" && guidanceController
    ? guidanceController.currentDescriptor() : null;
  const stage = campTutorialEtapeEffective();
  ...
  if (retirerKittyDeSesRoles(kittyIdx) === false) return false;
  const slot = slotRecette(familyId, slotIdx);
...
function ouvrirModalRecette(familyId, slotIdx) {
  if (typeof campTutorialAutoriserIntent !== "undefined"
      && !campTutorialAutoriserIntent({ type: "recipe", familyId: familyId, slotIndex: slotIdx })) return false;
  recipeModalOuvert = {
    familyId: familyId,
    slotIdx: slotIdx,
...
function appliquerSelectionRecette(familyId, slotIdx, recipeId) {
  ...
  const assignmentTutorielAConserver = campTutorialActif()
    && !slot.recipeId && Number.isInteger(slot.kittyIndex) && slot.kittyIndex >= 0;
  if (!assignmentTutorielAConserver) viderProgressionRecette(slot);
...
function assignerWorkerSlot(kittyIndex) {
  ...
  if (typeof campTutorialAutoriserIntent !== "undefined" && !campTutorialAutoriserIntent({
    type: "worker",
...
function verifierDeblocageWoodCathouseApresFabrication(resultatsRecettes, options) {
  const resultat = resultatsRecettes && resultatsRecettes.basicWoodPlanks;
  const progression = progressionCamp();
  if (progression.woodCathouseUnlocked === true || !resultat || !(resultat.produced > 0)) return false;
  progression.woodCathouseUnlocked = true;
  if (!(options && options.silent)) {
    ajouterLog("unlock", "Wood Cathouse unlocked after producing the first Basic Wood Plank.");
    afficherNotification("Wood Cathouse unlocked in Houses!");
  }
  return true;
}
...
function simulerTickHorsLigne(dt) {
  ...
  verifierDeblocageWoodCathouseApresFabrication(resultatsRecettes, { silent: true });
...
function campTutorialReconciliation() {
  const progression = progressionCamp();
  const current = campTutorialStage();
  const derived = saveCore.deriverEtapeTutorielSawmill(etat, etat.camp || {}, progression);
  if (!SAWMILL_TUTORIAL_STAGES.includes(derived)
      || SAWMILL_TUTORIAL_STAGE_INDEX[derived] <= SAWMILL_TUTORIAL_STAGE_INDEX[current]) {
    return false;
  }
  progression.sawmillTutorialStage = derived;
  return true;
}

function campTutorialEtapeEffective() {
  const persisted = campTutorialStage();
  if (persisted === "inactive" || persisted === "complete") return persisted;
  const persistedIndex = SAWMILL_TUTORIAL_STAGE_INDEX[persisted];
  const first = campTutorialSlot("wood", 0);
  const second = campTutorialSlot("wood", 1);
  if (persistedIndex >= SAWMILL_TUTORIAL_STAGE_INDEX["wood-slot-1-recipe"]
      && !(first && first.recipeId)) return "wood-slot-1-recipe";
  if (persistedIndex >= SAWMILL_TUTORIAL_STAGE_INDEX["wood-slot-1-cat"]
      && !(first && Number.isInteger(first.kittyIndex) && first.kittyIndex >= 0)) {
    return "wood-slot-1-cat";
  }
  if (persistedIndex >= SAWMILL_TUTORIAL_STAGE_INDEX["wood-slot-2-recipe"]
      && !(second && second.recipeId)) return "wood-slot-2-recipe";
  if (persistedIndex >= SAWMILL_TUTORIAL_STAGE_INDEX["wood-slot-2-cat"]
      && !(second && Number.isInteger(second.kittyIndex) && second.kittyIndex >= 0
        && first && second.kittyIndex !== first.kittyIndex)) {
    return "wood-slot-2-cat";
  }
  return persisted;
}

function campTutorialSlotProtege(slot) {
  if (!campTutorialActif() || !slot) return false;
  const first = campTutorialSlot("wood", 0);
  const second = campTutorialSlot("wood", 1);
  return (slot === first || slot === second) && Boolean(slot.recipeId || slot.kittyIndex !== null);
}

function campTutorialAutoriserMutationSlot(slot) {
  if (!campTutorialSlotProtege(slot)) return true;
  afficherNotification("Finish the highlighted Sawmill step before changing this recipe slot.");
  return false;
}

function campTutorialKittyProtege(kittyIndex) {
  if (!campTutorialActif()) return false;
  return [campTutorialSlot("wood", 0), campTutorialSlot("wood", 1)].some(function(slot) {
    return slot && slot.kittyIndex === kittyIndex;
  });
}

function campTutorialIntentAttendu(intent) {
  if (!campTutorialActif()) return true;
  const stage = campTutorialEtapeEffective();
  if (intent.type === "navigation-work") {
    if (stage === "sawmill") return false;
    if (stage === "work-action") {
      return intent.buildingId === "sawmill" && intent.familyId === "wood";
    }
    return true;
  }
  if (intent.type === "recipe") {
    return (stage === "wood-slot-1-recipe" && intent.familyId === "wood" && intent.slotIndex === 0)
      || (stage === "wood-slot-2-recipe" && intent.familyId === "wood" && intent.slotIndex === 1);
  }
  if (intent.type === "worker") {
    return (stage === "wood-slot-1-cat" && intent.familyId === "wood" && intent.slotIndex === 0)
      || (stage === "wood-slot-2-cat" && intent.familyId === "wood" && intent.slotIndex === 1);
  }
  return true;
}

function campTutorialAutoriserIntent(intent) {
  if (campTutorialIntentAttendu(intent || {})) return true;
  afficherNotification("Finish the highlighted Sawmill step first.");
  return false;
}
...
function campTutorialPickerEtape() {
  return campTutorialEtapeEffective() === "wood-slot-1-cat"
    || campTutorialEtapeEffective() === "wood-slot-2-cat";
}

function campTutorialPickerCible() {
  return {
    familyId: "wood",
    slotIdx: campTutorialEtapeEffective() === "wood-slot-1-cat" ? 0 : 1
  };
}

function campTutorialRecipeEtape() {
  return campTutorialEtapeEffective() === "wood-slot-1-recipe"
    || campTutorialEtapeEffective() === "wood-slot-2-recipe";
}

function campTutorialRecipeCible() {
  return {
    familyId: "wood",
    slotIdx: campTutorialEtapeEffective() === "wood-slot-1-recipe" ? 0 : 1
  };
}
  ...
  return {
    ...
    onActionCommitted: function(action) {
      ...
      if ((stage === "wood-slot-1-recipe" || stage === "wood-slot-2-recipe")
        ...
        if (stage !== campTutorialStage()) return true;
...
function campTutorialMarquerCible(element, descriptor) {
  ...
  if (descriptor && descriptor.id === "sawmill") {
    element.setAttribute("aria-describedby", campTutorialEtapeWork(campTutorialEtapeEffective())
...
function fermerDialogueRapideCamp() {
  ...
  if (typeof mobileInputDiagnostic !== "undefined") {
    mobileInputDiagnostic.recordOutcome("quick-dialogue.continue", { dialogueId: id || null });
  }
  sauvegarder();
...
function appealIntroEnAttente() {
  ...
  return etat.chatons >= 7
    && (progression.appealUnlocked !== true || progression.appealIntroSeen !== true);
}

function synchroniserDeblocageAppealDepuisStory(modal) {
  if (!modal || modal.id !== "ecran-story-appeal") return false;
  const unlockBeat = modal.querySelector("#story-appeal-unlock-copy");
  const progression = progressionCamp();
  if (!unlockBeat || unlockBeat.hidden || progression.appealUnlocked === true) return false;
  progression.appealUnlocked = true;
  sauvegarder();
  rendu();
  return true;
}

function terminerIntroAppeal() {
  fermerModal("ecran-story-appeal");
  const progression = progressionCamp();
  progression.appealUnlocked = true;
  progression.appealIntroSeen = true;
  progression.appealRecruitConfirmationPending = false;
  appealIntroApresRecruit = false;
  marquerStoryVue("storyAppealVue");
  sauvegarder();
  rendu();
  renduStories();
}
...
function verifierStoryModals() {
  ...
  if (progression.appealRecruitConfirmationPending) {
    const seventhKitty = Array.isArray(etat.kittiesData) ? etat.kittiesData[6] : null;
    const recruitResult = document.getElementById("recruit-result-popup");
    if (!appealIntroApresRecruit && seventhKitty && (!recruitResult
        || recruitResult.getAttribute("aria-hidden") !== "false")) {
      ouvrirPopupRecruitResult(true, seventhKitty.nom, seventhKitty.visage);
    }
    return;
  }
...
function estBatimentProductionCamp(typeOrId) {
  const typeId = typeof typeOrId === "string"
...
function assurerEtatCampPrincipal() {
  ...
  if (!etat.camp.progression || typeof etat.camp.progression !== "object"
      || Array.isArray(etat.camp.progression)) {
    etat.camp.progression = base.progression;
  } else {
    Object.keys(base.progression).forEach(function(key) {
      if (etat.camp.progression[key] === undefined) {
        etat.camp.progression[key] = base.progression[key];
      }
    });
  }
  return etat.camp;
...
function validerAmeliorationCamp(uid) {
  ...
  libererWorkersBatimentProductionCamp(item.type);
  delete etat.camp.upgrades[uid];
...
function libererWorkersBatimentProductionCamp(typeId) {
  const familyId = CAMP_PROTOTYPE_WORK_FAMILY_BY_TYPE[typeId];
  const slots = familyId && etat.workRecipeSlots && etat.workRecipeSlots[familyId];
...
function cibleAccessiblePourNettoyageCamp(cible) {
  if (!cible) return false;
  if (cibleAccessibleDepuisCamp(cible)) return true;
  // Buildings keep their normal collision and access rules, but their placement
  // must never turn an otherwise reachable Junk into an inert target.
  const layoutSansBatimentsJoueur = campPrototypeLayout.filter(function(item) {
    const type = item && typeCampPrototype(item.type);
...
function peutDemolirCibleCampPrototype(cible) {
  ...
  if (cible.kind === "terrain") {
    ...
    return cibleAccessiblePourNettoyageCamp(cible)
...
function ouvrirWorkDepuisCamp(event) {
  ...
  if (!campTutorialAutoriserIntent({
    type: "navigation-work", buildingId: buildingId, familyId: famille
  })) return false;
  ...
  fermerMenuInteractionCampPrototype();
  changerOnglet("work", {
    tutorialWorkIntent: { buildingId: buildingId, familyId: famille },
    semanticCommit: false
  });
...
function modifierRoutesCampPrototype(cellules, effacer) {
  if (!campPrototypeModeEdition || (!DEV_MODE && !appealCampDebloque())) return false;
  const uniques = [];
  const dejaVues = new Set();
...
function changerOnglet(id, options) {
  ...
  if (id === "work") {
    const tutorialWorkIntent = options && options.tutorialWorkIntent || {};
    if (!campTutorialAutoriserIntent({
      type: "navigation-work",
      buildingId: tutorialWorkIntent.buildingId,
...
function fermerPopupRecruitResult() {
  ...
  if (progression.appealRecruitConfirmationPending
      || appealIntroApresRecruit || appealIntroEnAttente()) {
    progression.appealRecruitConfirmationPending = false;
    appealIntroApresRecruit = false;
    sauvegarder();
...
mobileInputDiagnostic.configure({
  ...
  getUiState: function() {
    return {
      ...
      tutorialStage: typeof campTutorialStage === "function" ? campTutorialStage() : null,
      campPendingGesture: {
        boardPointer: campPrototypePointeur ? {
          pointerId: campPrototypePointeur.pointerId || null,
          mode: campPrototypePointeur.mode || null,
          uid: campPrototypePointeur.uid || null
        } : null,
        itemActivation: campPrototypeGesteActivationItem ? {
          pointerId: campPrototypeGesteActivationItem.pointerId || null,
          pointerType: campPrototypeGesteActivationItem.pointerType || null,
          uid: campPrototypeGesteActivationItem.uid || null
        } : null,
        menuPointer: campPrototypeActionMenuPointeur ? {
          pointerId: campPrototypeActionMenuPointeur.pointerId || null,
          action: campPrototypeActionMenuPointeur.action || null
        } : null,
        pinchActive: Boolean(campPrototypePincement)
      }
    };
  }
});
